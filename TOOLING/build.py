...
def validate_dist(mapping: dict) -> list:
    ...
    Uses the exact same "real reference" pattern as rewrite_references()
    above (a lookbehind for '/', '"', or "'" immediately before the
    filename, and a lookahead for '"', "'", or '?' immediately after) -
    not a bare substring check. The two functions sharing one definition
    of "real reference" means they can never disagree: anything this
    function would flag is, by construction, exactly the kind of
    reference rewrite_references() would have rewritten, so a clean
    validation result is a genuine guarantee, not a coincidence.

    A prior version of this function used a plain substring check
    ("if old_name in line"), reasoning that a hashed filename like
    "app.9022f63c3c.js" never contains "app.js" as a contiguous
    substring - true, but that only protects against false-positiving on
    the NEW hashed names. It did not protect against the OLD plain name
    appearing incidentally in a comment, in prose documentation, or in a
    dev-tooling script's own source code talking about app.js as a
    concept (e.g. "app.js's renderSiteHeader() function...") - none of
    which are real references that need rewriting, and none of which
    should fail a build. Anchoring on the same real-reference pattern the
    rewrite step already uses fixes this precisely, without weakening the
    check for genuine unversioned references (see the regression test
    this fix was validated against).
    ...
    problems = []
    reference_patterns = {
        old_name: re.compile(r'(?<=[/"\'])' + re.escape(old_name) + r'(?=["\'\?])')
    ...
    for path in DIST_DIR.rglob("*"):
        ...
        for lineno, line in enumerate(lines, start=1):
            for old_name, pattern in reference_patterns.items():
